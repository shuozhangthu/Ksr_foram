...
xpH_sw = -8.705e-6*xdepth+7.824;
xDIC_sw= -0.01209*xdepth+2334;
xTemp = -0.000436*xdepth+3.13;
P=1+xdepth/10;
xlnK_AB_P= log(10^(-8.39))+44.80748*P/83.15/275.05-13.46345*P.*P*0.001/2/83.15/275.05;
xlnK_hco3_P= log(10^(-10.5996)) +17.08581*P/83.15/274.51-4.87*P.*P*0.001/2/83.15/274.51;
xlnK_MB_P= xlnK_AB_P+2.19 +((P-1)*5.1*0.01/8.315/275.05);

xK_AB_sw=exp(xlnK_AB_P);
xK_MB_sw=exp(xlnK_MB_P);
xK_hco3_sw=exp(xlnK_hco3_P);
...
for ii = 1:length(xdepth)
    ...
    [o,time,out]=function_chen_depth_p(xdepth(ii),xpH_sw(ii),xDIC_sw(ii),xTemp(ii),xK_AB_sw(ii),xK_MB_sw(ii),xK_hco3_sw(ii));
