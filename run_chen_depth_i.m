...
xdepth = [2382:100:5000]';
xpH_sw = -2.473e-5*xdepth+7.932;
xDIC_sw= -0.01073*xdepth+2293;
xTemp = -0.0005567*xdepth+3.426;
P=1+xdepth/10;
xlnK_AB_I= log(10^(-8.384629))+44.98927*P/83.15/274.51-13.53068*P.*P*0.001/2/83.15/274.51;
xlnK_hco3_I= log(10^(-10.6077)) +17.08581*P/83.15/274.51-4.87*P.*P*0.001/2/83.15/274.51;
xlnK_MB_I= xlnK_AB_I+ 2.1854 +((P-1)*5.1*0.01/8.315/274.51);

xK_AB_sw=exp(xlnK_AB_I);
xK_MB_sw=exp(xlnK_MB_I);
xK_hco3_sw=exp(xlnK_hco3_I);
...
for ii = 1:length(xdepth)
    ...
    [o,time,out]=function_chen_depth_i(xdepth(ii),xpH_sw(ii),xDIC_sw(ii),xTemp(ii),xK_AB_sw(ii),xK_MB_sw(ii),xK_hco3_sw(ii));
