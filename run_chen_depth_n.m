...
xdepth = [1375:100:4900]';
xpH_sw = -3.828e-5*xdepth+8.175;
xDIC_sw= -0.002257*xdepth +2122;
xTemp = -4.423e-5*xdepth-0.7999;
P=1+xdepth/10;
xlnK_AB_N= log(10^(-8.37957))+ 45.778*P/83.15/272.26-13.81*P.*P*0.001/2/83.15/272.26;
xlnK_hco3_N= log(10^(-10.643)) +17.478559*P/83.15/272.26-4.87*P.*P*0.001/2/83.15/272.26;
xlnK_MB_N= xlnK_AB_N+ 2.2133 +((P-1)*5.1*0.01/8.315/272.26);

 xK_AB_sw=exp(xlnK_AB_N);
xK_MB_sw=exp(xlnK_MB_N);
xK_hco3_sw=exp(xlnK_hco3_N);
...
for ii = 1:length(xdepth)
    ...
    [o,time,out]=function_chen_depth_n(xdepth(ii),xpH_sw(ii),xDIC_sw(ii),xTemp(ii),xK_AB_sw(ii),xK_MB_sw(ii),xK_hco3_sw(ii));
